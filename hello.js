    ...
    ans.forEach(obj=>{
        promises.push(new Promise(function(resolve, reject){
            connection.query("SELECT secondUser FROM followers WHERE firstUser = ?", [obj.uemail], function (err, mas) {
                if (err) {console.log("111"); reject();}
                else {
                    obj.followers = mas.map(el=> {return el.secondUser});
                    resolve();
                }
            })
        }.bind(obj)));
        promises.push(new Promise(function(resolve, reject){
            connection.query("SELECT firstUser FROM followers WHERE secondUser = ?", [obj.uemail], function (err, mas) {
                if (err) {console.log("222"); reject();}
                else {
                    obj.following  = mas.map(el=> {return el.firstUser});
                    resolve();
                }
            });
        }.bind(obj)));
        promises.push(new Promise(function(resolve, reject) {
            connection.query("SELECT sthread FROM subscriptions  WHERE suser = ?", [obj.uemail], function (err, mas) {
                if (err) {console.log("333"); reject();}
                else {
                    obj.subscriptions = mas.map(el=> {return el.sthread});
                    resolve();
                }
            });
        }.bind(obj)));
...
app.post('/db/api/clear/', function (req, res) {
    res.set('Content-Type', 'application/json; charset=utf-8');
    connection.query(`DELETE FROM followers; DELETE FROM tree; 
...
app.get('/db/api/post/list/', function(req, res){
    ...
    connection.query(sql, function (err, ans) {
            if (!err) {
                ...
                return res.send({code: 0, response: ans.map(el => {return PostDetailsOut(el).response})});
            }
            else {
        ...
        connection.query(sql, function(err, ansArr) {
            ...
            if (withUser) {
                ...
                addDetailsInUserArray(ansArr, data=>{
                    ...
                    return res.send({
                        code: 0,
                        response: ansArr.map(el => {return ThreadDetailsOut(el).response})
                    });
        ...
        connection.query(sql, function(err, ans) {
            if (null !=ans) {
                ...
                addDetailsInUserArray(ans, (data)=>{
                    var outMas = data.map(el => {return UserDetailsOut(el).response});
                    var out = {
                        code: 0,
                        response: outMas
                    };
...
app.get('/db/api/user/listPosts/', function (req, res) {
    ...
    if (null !=user) {
        ...
        connection.query(sql, function(err, ans) {
            if (null !=ans) {
                var outArr = ans.map(el=>{return PostDetailsOut(el).response});
                var out = {
                    code: 0,
                    response: outArr
                };
